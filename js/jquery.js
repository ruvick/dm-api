...
// незащищенный в другом месте, кажется безопаснее определить глобальный только для этого модуля
...
jQuery.extend = jQuery.fn.extend = function () {
	параметры var, имя, источник, копия, copyIsArray, clone,
		цель = аргументы [0] || {},
		я = 1,
...
вар я,
	...
	runescape = new RegExp ("\\\\ [\\ da-fA-F] {1,6}" + пробел + «? | \\\\ ([^ \\ r \\ n \\ f])» , "г" ),
 ...
 * имя свойства - строка (с пробелом) и (если размер кеша превышает Expr.cacheLength)
		...
		if ("форма" в элем) {
			...
			if (elem.parentNode && elem.disabled === false) {
				...
				return elem.isDisabled === отключено ||
					...
					// Если isDisabled нет, проверяем вручную
					/ * jshint -W018 * /
...
setDocument = Sizzle.setDocument = function (node) {
	...
	Expr.find ["CLASS"] = support.getElementsByClassName && function (className, context) {
		if (typeof context.getElementsByClassName! == "undefined" && documentIsHTML) {
			return context.getElementsByClassName (имя класса);
		}
	};
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		assert (function (el) {
			...
			// Для выбора специально выбрана пустая строка
		...
		assert (function (el) {
			...
			// Поддержка: собственные приложения Windows 8
...
Выражение = Sizzle.selectors = {
	...
	preFilter: {
		"ATTR": функция (совпадение) {
			...
			// Перемещаем заданное значение в соответствие [3] вне зависимости от того, заключено оно в кавычки или нет
		...
		"ПСЕВДО": функция (совпадение) {
			...
			// Удаляем лишние символы из аргументов без кавычек
				...
				(Excess = unquoted.indexOf (")", unquoted.length - превышение) - unquoted.length)) {
	...
	filter: {
		...
		"РЕБЕНОК": функция (тип, что, _аргумент, первый, последний) {
			...
			вернуть первый === 1 && последний === 0?
				...
				function (elem, _context, xml) {
					var cache, uniqueCache, outerCache, node, nodeIndex, начало,
		...
		"ПСЕВДО": функция (псевдо, аргумент) {
			...
			// Расстановка приоритетов по чувствительности к регистру в случае добавления пользовательских псевдонимов с прописными буквами
	...
	псевдо: {
		...
		"not": markFunction (function (selector) {
			...
			вернуть сопоставитель [раскрыть]?
				markFunction (function (seed, match, _context, xml) {
					...
					в то время как я-- ) {
						if ((elem = unmatched [i])) {
							семя [я] =! (соответствует [я] = элемент);
						}
					}
				}):
				function (elem, _context, xml) {
		...
		"empty": function (elem) {
			...
			//: пусто отменяется элементом (1) или узлами содержимого (текст: 3; cdata: 4; ссылка на объект: 5),
	...
	вернуть комбинатор. сначала?
		...
		function (elem, context, xml) {
			...
			if (xml) {
				...
				while ((elem = elem [dir])) {
					if (elem.nodeType === 1 || checkNonElements) {
						externalCache = elem [развернуть] || (элемент [раскрыть] = {});
						...
						if (skip && skip === elem.nodeName.toLowerCase ()) {
							...
							// Соответствие означает, что мы закончили; неудача означает, что мы должны продолжать проверку
...
функция setMatcher (preFilter, selector, matcher, postFilter, postFinder, postSelector) {
	...
	return markFunction (function (seed, results, context, xml) {
		var temp, я, элемент,
			...
			matcherIn = preFilter && (семя ||! селектор)?
				конденсировать (elems, preMap, preFilter, context, xml):
				элементы

			matcherOut = matcher?

				// Если у нас есть postFinder, или отфильтрованное начальное число, или пост-фильтр, не являющийся начальным, или уже существующие результаты,
				postFinder || (seed? preFilter: ранее существовавший || postFilter)?
		...
		if (seed) {
			if (postFinder || preFilter) {
				if (postFinder) {
					...
					// Получаем финальный matcherOut путем сжатия этого промежуточного звена в контексты postFinder
					temp = [];
					я = matcherOut.length;
					в то время как я-- ) {
					...
					postFinder (ноль, (matcherOut = []), темп, xml);
				}
...
function matcherFromTokens (tokens) {
	var checkContext, сопоставитель, j,
		...
		matchers = [function (elem, context, xml) {
			...
			// Избегайте зависания элемента (проблема # 299)
			checkContext = null;
			return ret;
		}];
		...
		superMatcher = function (seed, context, xml, results, external) {
			...
			if (bySet && i! == matchedCount) {
				...
				while ((matcher = setMatchers [j ++])) {
					сопоставитель (несоответствие, setMatched, context, xml);
				}
...
select = Sizzle.select = function (selector, context, results, seed) {
	...
	if (match.length === 1) {
		...
		if (tokens.length> 2 && (token = tokens [0]) .type === "ID" &&
			...
			// Предварительно скомпилированные сопоставители по-прежнему будут проверять происхождение, поэтому переходите на следующий уровень
		...
		в то время как я-- ) {
			...
			if ((find = Expr.find [type])) {
				...
				если ((семя = найти (
					...
					// Если seed пуст или токенов не осталось, мы можем вернуться раньше
					tokens.splice (i, 1);
...
support.sortDetached = assert (function (el) {
	...
	// Должно возвращать 1, но возвращает 4 (после)
...
if (! assert (function (el) {
	...
	addHandle (логические, функция (elem, name, isXML) {
		var val;
		if (! isXML) {
			return elem [name] === правда? name.toLowerCase ():
				(val = elem.getAttributeNode (имя)) && val.specified?
					val.value:
					значение NULL;
		}
				...
				if (match [1]) {
					...
					if (rsingleTag.test (match [1]) && jQuery.isPlainObject (context)) {
						for (соответствие в контексте) {
							...
							// Свойства контекста по возможности называются методами
...
jQuery.each ({
	parent: function (elem) {
		...
		вернуть родительский && parent.nodeType! == 11? родитель: ноль;
	},
	родители: function (elem) {
		return dir (elem, "parentNode");
	},
	parentUntil: function (elem, _i, до) {
		return dir (elem, "parentNode", до);
	},
	следующий: function (elem) {
		return sibling (elem, "nextSibling");
	},
	prev: function (elem) {
		return sibling (elem, «предыдущий брат»);
	},
	...
	content: function (elem) {
		если (elem.contentDocument! = null &&
			...
			// `contentDocument` с прототипом` null`.
	...
	jQuery.fn [имя] = функция (до, селектор) {
		var matched = jQuery.map (это, fn, до);

		if (name.slice (-5)! == "До") {
			селектор = до ;
		}

		if (selector && typeof selector === "строка") {
			matched = jQuery.filter (селектор, сопоставлен);
		}

		if (this.length> 1) {

			// Удаляем дубликаты
			if (! GuaraniqueUnique [имя]) {
...
jQuery.Callbacks = function (options) {
	...
	var // Флаг, чтобы узнать, активен ли список в данный момент
		...
		// Индекс текущего обратного вызова (изменяется добавлением / удалением по мере необходимости)
		firingIndex = -1,
		...
		fire = function () {
			...
			// Убираем, если мы закончили стрельбу навсегда
			if (заблокировано) {
				...
				if (list) {
					...
					(функция add (args) {
						jQuery.each (args, function (_, arg) {
							...
							} else if (arg && arg.length && toType (arg)! == "строка") {
...
функция acceptValue (значение, разрешение, отклонить, noValue) {
	var метод;

	пытаться {

		// Сначала проверяем аспект обещания, чтобы получить привилегию синхронного поведения
		if (значение && isFunction ((method = value.promise))) {
			method.call (значение) .done (разрешить) .fail (отклонить);

		// Другие предметы
		} else if (value && isFunction ((method = value.then))) {
			method.call (значение, разрешить, отклонить);

		// Прочие небогатые
		} else {

			// Управление аргументами `resolve`, позволяя Array # slice приводить логическое значение` noValue` к целому числу:
			// * false: [значение] .slice (0) => resolve (значение)
			// * true: [значение] .slice (1) => resolve ()
			resolve.apply (undefined, [значение] .slice (noValue));
		}

	// Для Promises / A + преобразуем исключения в отклонения
	// Поскольку jQuery.when не разворачивает файлы, мы можем пропустить дополнительные проверки, появляющиеся в
	// Отложено #, чтобы условно подавить отклонение.
		...
		reject.apply (undefined, [значение]);
	}
}

jQuery.extend ({

	Отложено: function (func) {
		var tuples = [
				...
				[«решить», «готово», jQuery.Callbacks («однажды запомнить»),
					jQuery.Callbacks («однажды память»), 0, «разрешено»],
				[«отклонить», «сбой», jQuery.Callbacks («однажды запомнить»),
			...
			обещание = {
				...
				pipe: function (/ * fnDone, fnFail, fnProgress * /) {
					...
					return jQuery.Deferred (function (newDefer) {
						jQuery.each (кортежи, функция (_i, кортеж) {
							...
							// Сопоставляем кортежи (прогресс, выполнение, сбой) с аргументами (выполнено, сбой, прогресс)
							var fn = isFunction (fns [кортеж [4]]) && fns [кортеж [4]];

							// deferred.progress (function () {привязать к newDefer или newDefer.notify})
							// deferred.done (function () {привязать к newDefer или newDefer.resolve})
							// deferred.fail (function () {привязать к newDefer или newDefer.reject})
				...
				затем: function (onFulfilled, onRejected, onProgress) {
					var maxDepth = 0;
					функция resolve (глубина, отложенная, обработчик, специальная) {
						return function () {
							var that = this,
								args = аргументы,
								mightThrow = function () {
									...
									вернулся = handler.apply (это, аргументы);

									// Поддержка: Promises / A + раздел 2.3.1
									// https://promisesaplus.com/#point-48
									if (вернул === deferred.promise ()) {
										throw new TypeError ("Возможное саморазрешение");
									}
									...
									затем = вернулся &&
										...
										(typeof вернул === "объект" ||
											typeof вернул === "функция") &&
										вернулся. затем;

									// Обработка возвращаемого объекта
									if (isFunction (then)) {

										// Специальные процессоры (уведомление) просто ждут разрешения
										if (special) {
											then.call (
												вернулся,
												разрешение (maxDepth, отложенный, удостоверение, специальный),
												разрешение (maxDepth, отложенный, Thrower, специальный)
											);

										// Нормальные процессоры (разрешение) также подключаются к прогрессу
										} else {
											...
											then.call (
												...
												разрешение (maxDepth, отложенный, удостоверение, специальный),
												разрешение (maxDepth, отложенный, Thrower, специальный),
												разрешение (maxDepth, отложенный, удостоверение,
													deferred.notifyWith)
											);
										}
									...
									// Обрабатываем все остальные возвращаемые значения
										...
										// Только обработчики замены передают контекст
										// и несколько значений (поведение вне спецификации)
										if (обработчик! == Identity) {
											that = undefined;
											args = [возвращено];
										}

										// Обработка значения (я)
										// Процесс по умолчанию - разрешение
										(специальный || deferred.resolveWith) (что, аргументы);
									}
								},

								// Только обычные процессоры (разрешают) ловят и отклоняют исключения
								процесс = особенный?
									mightThrow:
									function () {
										пытаться {
							...
							// Повторное разрешение обещаний немедленно, чтобы избежать ложного отказа от
							// последующие ошибки
							if (depth) {
								процесс();
							} else {

								// Вызов дополнительной ловушки для записи стека в случае исключения
								// так как иначе он теряется, когда выполнение идет асинхронно
				...
				обещание: function (obj) {
					вернуть obj! = null? jQuery.extend (obj, обещание): обещание;
				}
			},
			отложенный = {};
		...
		jQuery.each (кортежи, функция (я, кортеж) {
			...
			// обещание.done = list.add
			// обещание.fail = list.add
			...
			if (stateString) {
				list.add (
					function () {
						...
						// состояние = "отклонено"
						состояние = stateString;
					},

					// rejected_callbacks.disable
					// выполнено_callbacks.disable
			...
			отложенный [кортеж [0]] = функция () {
				отложенный [кортеж [0] + "С"] (это === отложенный? undefined: это, аргументы);
				вернуть это;
			};
		...
		if (func) {
			func.call (отложенный, отложенный);
		}
	...
	when: function (singleValue) {
		...
		// Одинарные и пустые аргументы принимаются как Promise.resolve
		если (осталось <= 1) {
			acceptValue (singleValue, master.done (updateFunc (i)) .resolve, master.reject,
				!осталось );

			// Используйте .then () для разворачивания вторичных файлов (см. Gh-3000)
			if (master.state () === "ожидание" ||
		...
		вернуть master.promise ();
	}
});


// Обычно это указывает на ошибку программиста во время разработки,
// предупредить о них как можно скорее, а не проглатывать по умолчанию.
var rerrorNames = / ^ (Eval | Internal | Range | Reference | Syntax | Type | URI) Error $ /;

jQuery.Deferred.exceptionHook = function (error, stack) {

	// Поддержка: только IE 8 - 9
	// Консоль существует, когда открыты инструменты разработчика, что может произойти в любое время
...
// Отложенный, используемый в DOM, готов
...
jQuery.fn.ready = function (fn) {
	...
	готовый список
		...
		// Оборачиваем jQuery.readyException в функцию, чтобы поиск
...
jQuery.extend ({
	...
	// Готова ли модель DOM к использованию? Установите значение true, как только это произойдет.
	isReady: ложь,

	// Счетчик для отслеживания, сколько элементов ждать перед
	...
	готово: функция (подождите) {
		...
		// Прервать, если есть отложенные удержания или мы уже готовы
		...
		// Если сработало обычное событие DOM Ready, уменьшаем и ждем, если нужно
		if (wait! == true && --jQuery.readyWait> 0) {
			возвращение;
		}

		// Если есть привязанные функции, выполнить
...
// Поймать случаи, когда вызывается $ (document) .ready ()
...
if (document.readyState === "complete" ||
	...
	// Обрабатываем его асинхронно, чтобы позволить скриптам отложить готовность
...
// Значение / s может быть выполнено по желанию, если это функция
...
Data.prototype = {
	...
	cache: function (owner) {
		...
		// Проверяем, есть ли у объекта-владельца уже кеш
		значение переменной = владелец [this.expando];
		...
		if (! value) {
			...
			if (acceptData (owner)) {
				...
				// В противном случае закрепите его неперечислимым свойством
				// настраиваемый должен быть истинным, чтобы свойство могло быть
	...
	set: function (владелец, данные, значение) {
		...
		// Дескриптор: [владелец, ключ, значение] аргументы
		// Всегда используйте ключ camelCase (gh-2257)
		if (typeof data === "строка") {
			кеш [camelCase (данные)] = значение;

		// Дескриптор: [владелец, {свойства}] аргументы
		} else {

			// Копируем свойства по одному в объект кеша
			for (опора в данных) {
				кеш [camelCase (опора)] = данные [опора];
			}
		}
		вернуть кеш;
	},
	get: function (owner, key) {
		ключ возврата === undefined?
			this.cache (владелец):
	...
	доступ: функция (владелец, ключ, значение) {
		...
		//
		// 1. Ключ не указан
		// 2. Был указан строковый ключ, но не указано значение
		//
		// Выбираем путь "чтения" и позволяем методу get определять
		// какое значение вернуть, соответственно либо:
		//
		// 1. Весь объект кеша
		// 2. Данные, хранящиеся на ключе
		//
		если (ключ === undefined ||
				((key && typeof key === "строка") && value === undefined)) {
		...
		// Поскольку у "установленного" пути может быть две возможные точки входа
		// возвращаем ожидаемые данные на основе выбранного пути [*]
	...
	remove: function (owner, key) {
		...
		// Удаляем раскрытие, если данных больше нет
	...
	hasData: function (owner) {
		...
		вернуть кеш! == undefined &&! jQuery.isEmptyObject (кеш);
	}
};
...
// Сводка реализации
//
// 1. Обеспечить поверхность API и семантическую совместимость с веткой 1.9.x
// 2. Повышение ремонтопригодности модуля за счет уменьшения объема памяти
// пути к одному механизму.
// 3. Используйте один и тот же механизм для поддержки «частных» и «пользовательских» данных.
// 4. _Никогда_ не предоставлять "личные" данные коду пользователя (TODO: Drop _data, _removeData)
// 5. Избегайте раскрытия деталей реализации для пользовательских объектов (например, свойств expando)
// 6. Укажите четкий путь для внедрения обновления до WeakMap в 2014 г.
...
функция dataAttr (elem, key, data) {
	...
	// Если ничего не было найдено внутри, пытаемся получить любой
...
jQuery.extend ({
	hasData: function (elem) {
		вернуть dataUser.hasData (elem) || dataPriv.hasData (elem);
	},

	data: function (elem, name, data) {
		вернуть dataUser.access (elem, name, data);
	},
	...
	// с прямыми вызовами методов dataPriv их можно исключить.
...
jQuery.fn.extend ({
	data: function (ключ, значение) {
		...
		if (key === undefined) {
			if (this.length) {
				...
				if (elem.nodeType === 1 &&! dataPriv.get (elem, "hasDataAttrs")) {
					...
					dataPriv.set (elem, "hasDataAttrs", истина);
				}
			}
		...
		return access (this, function (value) {
			...
			// Вызывающий объект jQuery (элементы совпадают) не пуст
			// (и поэтому элемент появляется в этом [0]) и
			// Параметр `value` не был неопределенным. Пустой объект jQuery
			// приведет к `undefined` для elem = this [0], что приведет к
			// генерируем исключение, если сделана попытка чтения кеша данных.
			if (elem && value === undefined) {

				// Попытка получить данные из кеша
				// Ключ всегда будет в формате CamelCased в Data
...
jQuery.extend ({
	очередь: функция (элемент, тип, данные) {
		...
		if (elem) {
			type = (type || «fx») + «очередь»;
			очередь = dataPriv.get (элемент, тип);

			// Ускорение удаления из очереди за счет быстрого выхода, если это просто поиск
			if (data) {
				if (! очередь || Array.isArray (данные)) {
	...
	dequeue: function (elem, type) {
		...
		// Если очередь fx исключена, всегда удаляем индикатор прогресса
		if (fn === "inprogress") {
			fn = queue.shift ();
			startLength--;
		}
		...
		if (fn) {
			...
			// Добавляем индикатор прогресса, чтобы предотвратить появление очереди fx
			// автоматически удаляется из очереди
			if (type === "fx") {
				queue.unshift («в процессе»);
			}
	...
	_queueHooks: function (elem, type) {
		...
		вернуть dataPriv.get (elem, key) || dataPriv.access (elem, key, {
...
jQuery.fn.extend ({
	queue: function (type, data) {
		...
		if (arguments.length <установщик) {
			return jQuery.queue (this [0], тип);
		}

		вернуть данные === undefined?
			этот :
			this.each (function () {
				var queue = jQuery.queue (это, тип, данные);

				// Обеспечиваем хуки для этой очереди
				jQuery._queueHooks (это, тип);

				if (введите === "fx" && queue [0]! == "inprogress") {
	...
	// Получаем выполнение обещания, когда очереди определенного типа
	// опорожняются (по умолчанию тип fx)
...
var pnum = (/[+- ]?(?:\d*\.|)\d+(?:[eE sizes[+-pting?\d+|)/) .source;
...
var documentElement = document.documentElement;
	...
	var isAttached = function (elem) {
		...
		Состав = {Состав: истина};

	// Поддержка: IE 9 - 11+, Edge 12 - 18+, только iOS 10.0 - 10.2
	// Проверяем прикрепление через границы теневого DOM, когда это возможно (gh-3504)
	// Поддержка: только iOS 10.0-10.2
	// Ранние версии iOS 10 поддерживают `attachShadow`, но не` getRootNode`,
	// приводящие к ошибкам. Нам нужно проверить getRootNode.
	if (documentElement.getRootNode) {
		isAttached = function (elem) {
			return jQuery.contains (elem.ownerDocument, elem) ||
				elem.getRootNode (в составе) === elem.ownerDocument;
		};
	}
var isHiddenWithinTree = function (elem, el) {

		// isHiddenWithinTree может быть вызван из jQuery # filter function;
		// в этом случае вторым аргументом будет элемент
		elem = el || элем;
		...
		return elem.style.display === "none" ||
			...
			// Отсоединенные элементы могут иметь вычисленное display: none, поэтому сначала убедитесь, что elem
			// в документе.
			isAttached (elem) &&

			jQuery.css (elem, «дисплей») === «нет»;
	};
...
function adjustCSS (elem, prop, valueParts, tween) {
	вар скорректирован, масштаб,
		...
		начальный = currentValue (),
		unit = valueParts && valueParts [3] || (jQuery.cssNumber [prop]? "": "px"),

		// Расчет начального значения необходим для возможных несоответствий единиц измерения
		initialInUnit = elem.nodeType &&
			(jQuery.cssNumber [prop] || unit! == "px" && + начальный) &&
			rcssNum.exec (jQuery.css (elem, prop));

	if (initialInUnit && initialInUnit [3]! == unit) {

		// Поддержка: Firefox <= 54
		// Уменьшить вдвое целевое значение итерации, чтобы предотвратить помехи от верхних границ CSS (gh-2144)
		начальная = начальная / 2;

		// Единицы доверия, сообщаемые jQuery.css
		...
		while (maxIterations--) {
			...
			// Оцениваем и обновляем наше лучшее предположение (удвоение предположений обнуляет).
		...
		// Убедитесь, что мы обновим свойства анимации движения позже
	...
	if (valueParts) {
		...
		if (tween) {
			...
			tween.end = отрегулировано;
		}
	}
	доход скорректирован;
}
